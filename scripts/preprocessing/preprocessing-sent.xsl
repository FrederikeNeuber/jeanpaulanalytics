...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="output-cleaned"
            select="replace($input-cleaned, 'data-source/cab2', 'data-analysis')" as="xs:string"/>
        ...
        <xsl:for-each
            select="$documents//tei:TEI[//tei:correspAction[@type = 'sent']/tei:persName/@key = 'JP-004393']">
            ...
            <xsl:result-document
                href="{$output-uri || '/sentiment/minnaspazier/' || $file || '.txt'}" method="text"
