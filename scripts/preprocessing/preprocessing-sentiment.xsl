...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="output-cleaned" select="replace($input-cleaned, 'data-source/cab', 'data-preprocessed/text-by-keyword')" as="xs:string"/>
        ...
        <xsl:for-each-group select="$documents//tei:TEI"
            ...
            <xsl:result-document href="{$output-uri || 'letters-plain-sentiment/correspondents/' ||  replace($name, ' ', '-') || '_' || $filename  || '.txt'}"
        ...
        <xsl:for-each-group select="$documents//tei:TEI"
            ...
            <xsl:variable name="filename" select="current-grouping-key()"/>
            <xsl:result-document href="{$output-uri || 'letters-plain-sentiment/years/' || $filename || '.txt'}"
                method="text" indent="no" media-type="text">
                <xsl:apply-templates select="current-group()//@word"/>
            </xsl:result-document>
        </xsl:for-each-group>
        ...
        <xsl:for-each-group select="$documents//tei:TEI"
            ...
            <xsl:result-document href="{$output-uri || 'letters-plain-sentiment/keywords/' || replace(replace($id, ' ', '-'), '/', '-') || '.txt'}"
                method="text" indent="no" media-type="text">
                <xsl:apply-templates select="current-group()//@word"/>
            </xsl:result-document>
        </xsl:for-each-group>
